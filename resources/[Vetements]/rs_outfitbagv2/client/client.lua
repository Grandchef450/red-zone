                ...
                onSelect = function()
                    ...
                    elseif framework == "qb" then
                        QBCore.Functions.TriggerCallback("rs_outfitbag:getOutfitCount", function(count)
                            ...
                            local dialog = lib.inputDialog(Language.saveoutfit, {
                                { type = "input", label = Language.outfitname, placeholder = Language.myoutfit }
                            })
                            ...
                            if dialog and dialog[1] then
                                ...
                                TriggerServerEvent("rs_outfitbag:saveOutfit", dialog[1], outfit)
                                Notify(Language.title, Language.savedoutfit .. dialog[1], 'success')
                            end
                        end)
...
AddEventHandler('rs_outfitbag:showOutfitList', function()
    local callback = function(outfits)
    ...
    if framework == "esx" then
        ESX.TriggerServerCallback("rs_outfitbag:getOutfits", callback)
    elseif framework == "qb" then
        QBCore.Functions.TriggerCallback("rs_outfitbag:getOutfits", callback)
    end
end)
