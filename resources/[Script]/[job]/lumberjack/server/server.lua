...
AddEventHandler('gfx:server:lumberjack:CuttingTrees', function(cuttingTrees, bonus)
    if QBCore ~= nil then
        ...
        if result[1] ~= nil then
            ...
            -- ═══ REDZONE : DU BOIS, PAS DE L'ARGENT ═══════════════
            -- L'économie du serveur ne tourne pas au dollar mais aux
            -- matériaux. Abattre un arbre rapporte donc des morceaux
            -- de bois, qui alimentent tout l'arbre de craft — depuis
            -- le couteau de survie jusqu'au BBQ.
            --
            -- `money` reste le multiplicateur de rendement : le bonus
            -- de contrat s'applique donc bien à la quantité de bois.
            local bois = math.max(1, math.floor(money / 10))
            exports.ox_inventory:AddItem(src, 'morceau_bois', bois)

            TriggerClientEvent('gfx:client:lumberjack:Notify', src,
                ('%d morceau(x) de bois récupéré(s)'):format(bois))
...
AddEventHandler('gfx:server:lumberjack:GainContractMoney', function(treeCount, axebonus, percent, contractOwnerid)
    if QBCore ~= nil then
        ...
        if result[1] ~= nil then
            ...
            -- REDZONE : contrat payé en bois, comme l'abattage libre
            local boisContrat = math.max(1, math.floor(gainmoney / 10))
            exports.ox_inventory:AddItem(src, 'morceau_bois', boisContrat)

            TriggerClientEvent('gfx:client:lumberjack:Notify', src,
                ('%d morceau(x) de bois — contrat rempli'):format(boisContrat))
            --print("current level:", result[1].level, "new level:", level,result[1].cuttingTrees / Config.lumberjack.levelUpLimit)
            ...
            if owner then
                -- REDZONE : la part du commanditaire aussi en bois.
                -- Passer un contrat devient un moyen de faire farmer
                -- les autres pour soi, ce qui a du sens entre clans.
                local boisOwner = math.max(1, math.floor(ownerMoney / 10))
                exports.ox_inventory:AddItem(owner.source, 'morceau_bois', boisOwner)

                TriggerClientEvent('gfx:client:lumberjack:Notify', owner.source,
                    ('%d morceau(x) de bois — part de contrat'):format(boisOwner))
            else
...
-- REDZONE : cette fonction de secours versait de l'argent quand la
-- fiche du bûcheron était introuvable en base. Elle donne du bois
-- comme le reste, pour qu'aucun chemin ne verse de dollars.
--
-- Au passage : l'appel QBCore.Functions.AddMoney d'origine était
-- bogué — c'est une méthode du JOUEUR, pas du core. Il n'a jamais
-- rien versé à personne.
function GiveMoney(money, src)
    local bois = math.max(1, math.floor(money / 10))
    exports.ox_inventory:AddItem(src, 'morceau_bois', bois)

    TriggerClientEvent('gfx:client:lumberjack:Notify', src,
        ('%d morceau(x) de bois'):format(bois))
end
