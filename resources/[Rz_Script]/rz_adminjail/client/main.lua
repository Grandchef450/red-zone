...
local function dbg(msg)
    if Config.Debug then
        print(('^5[rz_adminjail]^7 %s'):format(msg))
    end
end
...
RegisterCommand('adminjail', function()
    ...
    TriggerServerEvent('rz_adminjail:server:requestOpen')
end, false)
RegisterKeyMapping('adminjail', 'Admin Jail (RZ)', 'keyboard', Config.OpenKey)

RegisterNetEvent('rz_adminjail:client:openPanel', function(data)
    panelOpen = true
...
RegisterNetEvent('rz_adminjail:client:panelData', function(data)
    if not panelOpen then return end
    local pos = GetEntityCoords(PlayerPedId())
    data.myPos = { x = pos.x, y = pos.y, z = pos.z, heading = GetEntityHeading(PlayerPedId()) }
...
RegisterNUICallback('jail', function(data, cb)
    TriggerServerEvent('rz_adminjail:server:jail', data)
    cb('ok')
end)

RegisterNUICallback('unjail', function(data, cb)
    TriggerServerEvent('rz_adminjail:server:unjail', tostring(data.license or ''))
    cb('ok')
end)

RegisterNUICallback('refresh', function(_, cb)
    TriggerServerEvent('rz_adminjail:server:refresh')
    cb('ok')
end)
...
RegisterNetEvent('rz_adminjail:client:jail', function(data)
    jailed   = true
...
RegisterNetEvent('rz_adminjail:client:release', function(releasePoint)
...
print('^2[rz_adminjail]^7 client charge — F11 / commande "adminjail"')

-- ═══════════════════════════════════════════════════════════════════
--  OUVERTURE DEPUIS LE MENU ADMIN (F5)
--
--  Le panneau admin appelle https://rz_adminjail/openPanel. On relaie sur
