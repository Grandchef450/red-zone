...
local function dbg(msg)
    if Config.Debug then print(('^3[vgc_props]^7 %s'):format(msg)) end
end
...
RegisterNetEvent('vgc_props:client:sync', function(list)
    syncProps(list)
end)

RegisterNetEvent('vgc_props:client:added', function(p)
    dbg(('prop place enregistre : [%d] %s'):format(p.id, p.label))
end)

RegisterNetEvent('vgc_props:client:error', function(msg)
...
CreateThread(function()
    ...
    Wait(500)
    TriggerServerEvent('vgc_props:server:request')
end)
...
print('^2[vgc_props]^7 client charge — /props ou ' .. Config.OpenKey)
...
RegisterNUICallback('startPlace', function(data, cb)
    ...
    CreateThread(function()
        ...
        if res then
            TriggerServerEvent('vgc_props:server:add', {
...
RegisterNUICallback('rename', function(data, cb)
    if data.id and type(data.label) == 'string' then
        TriggerServerEvent('vgc_props:server:rename', data.id, data.label)
    end
    cb('ok')
end)

RegisterNUICallback('delete', function(data, cb)
    if data.id then TriggerServerEvent('vgc_props:server:delete', data.id) end
    cb('ok')
end)
