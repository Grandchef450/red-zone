-- ============================================================
--  VGC VIP | server/main.lua — QBCore, persistance JSON (0 DB)
...
local function dbg(msg)
    if Config.Debug then print(('^5[vgc_vip:sv]^7 %s'):format(msg)) end
end
...
local function loadDb()
    ...
    if raw and raw ~= '' then
        ...
        print('^1[vgc_vip:sv]^7 vip.json corrompu — reinitialise en memoire (non ecrase avant prochaine sauvegarde)')
    end
    db = { players = {} }
end
...
local function saveDb()
    if not SaveResourceFile(RES, Config.VipFile, json.encode(db, { indent = true }), -1) then
        print('^1[vgc_vip:sv]^7 ECHEC ecriture vip.json')
    end
end
...
local function notifyResult(src, ok, msg)
    TriggerClientEvent('vgc_vip:client:result', src, { ok = ok, msg = msg })
end
...
local function pushState(src, Player)
    TriggerClientEvent('vgc_vip:client:state', src, buildState(src, Player))
end
...
RegisterNetEvent('vgc_vip:server:requestOpen', function()
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    TriggerClientEvent('vgc_vip:client:open', src, buildState(src, Player))
end)

RegisterNetEvent('vgc_vip:server:refresh', function()
    local src = source
    local Player = getPlayer(src)
...
RegisterNetEvent('vgc_vip:server:buyTier', function(tierId)
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    ...
    print(('^2[vgc_vip:sv]^7 %s (%s) a achete %s pour %d$'):format(Player.PlayerData.name or '?', cidOf(Player), tier.label, price))
...
RegisterNetEvent('vgc_vip:server:choosePed', function(model)
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
...
RegisterNetEvent('vgc_vip:server:applyPed', function(model)
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    model = tostring(model or '')
    ...
    TriggerClientEvent('vgc_vip:client:applyPed', src, model)
...
RegisterNetEvent('vgc_vip:server:useFreeRevive', function()
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    local rec  = getRecord(cidOf(Player))
...
RegisterNetEvent('vgc_vip:server:craftKit', function()
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    local rec  = getRecord(cidOf(Player))
...
RegisterNetEvent('vgc_vip:server:useKit', function()
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    local rec  = getRecord(cidOf(Player))
...
RegisterNetEvent('vgc_vip:server:buyShopItem', function(itemId)
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    ...
    elseif entry.give.type == 'revive_hospital' then
        TriggerClientEvent('vgc_vip:client:reviveHospital', src, Config.Hospital)
    end

    print(('^2[vgc_vip:sv]^7 %s a achete "%s" en materiaux'):format(Player.PlayerData.name or '?', entry.label))
...
RegisterNetEvent('vgc_vip:server:farm', function(spotIndex)
    local src = source
    local Player = getPlayer(src)
    if not Player then return end
    ...
    if dist > 5.0 then
        print(('^1[vgc_vip:sv]^7 farm refuse (distance %.1fm) pour %s'):format(dist, Player.PlayerData.name or src))
        return
    end
    ...
    TriggerClientEvent('vgc_vip:client:farmed', src, spot.material, amount)
end)
...
-- ============================================================
--  ADMIN — /setvip <idServeur> <grade 0-7> (console ou ace vgc.vip)
-- ============================================================

RegisterCommand('setvip', function(source, args)
    if source ~= 0 and not IsPlayerAceAllowed(source, 'vgc.vip') then return end
    ...
    saveDb()
    print(('^2[vgc_vip:sv]^7 setvip : %s → grade %d'):format(Player.PlayerData.name or '?', tierId))
