-- ============================================================
--  VGC ADMIN JAIL | server/main.lua
--  • Permissions : lit admins.json de vgc_admintablet (memes
...
local function dbg(msg)
    if Config.Debug then
        print(('^5[vgc_adminjail:sv]^7 %s'):format(msg))
    end
end
...
-- Lit les admins de vgc_admintablet a chaque verification :
...
local function releasePlayer(license, silent)
    ...
    if src then
        TriggerClientEvent('vgc_adminjail:client:release', src, Config.ReleasePoint)
        if not silent then
            notify(src, 'Peine terminée — bienvenue en ville.', true)
        end
    end
    print(('^2[vgc_adminjail:sv]^7 LIBERE : %s (%s)'):format(j.name or '?', license))
end
...
local function jailPlayer(adminSrc, targetSrc, seconds, coords, music, reason)
    ...
    TriggerClientEvent('vgc_adminjail:client:jail', targetSrc, {
        coords  = coords,
        seconds = seconds,
        music   = music,
        reason  = reason,
    ...
    print(('^3[vgc_adminjail:sv]^7 JAIL : %s (%s) — %ds par %s — raison: %s')
...
AddEventHandler('playerJoining', function()
    ...
    SetTimeout(8000, function()
        if GetPlayerName(src) then
            TriggerClientEvent('vgc_adminjail:client:jail', src, {
                coords  = j.coords,
...
RegisterNetEvent('vgc_adminjail:server:requestOpen', function()
    local src = source
    if not isAllowed(src) then
        notify(src, 'Accès refusé.', false)
        return
    end
    TriggerClientEvent('vgc_adminjail:client:openPanel', src, buildPanelData())
end)

RegisterNetEvent('vgc_adminjail:server:refresh', function()
    local src = source
    if not isAllowed(src) then return end
    TriggerClientEvent('vgc_adminjail:client:panelData', src, buildPanelData())
end)

RegisterNetEvent('vgc_adminjail:server:jail', function(data)
    local src = source
    if not isAllowed(src) then
        print(('^1[vgc_adminjail:sv]^7 TENTATIVE jail non autorisee par %s (id %d)'):format(GetPlayerName(src) or '?', src))
        return
    end
...
RegisterNetEvent('vgc_adminjail:server:unjail', function(license)
    local src = source
    if not isAllowed(src) then return end
