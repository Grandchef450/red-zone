-- ============================================================
--  RZ JOB CREATOR  |  server/main.lua
...
local function dbg(msg)
    if Config.Debug then
        print(('^6[rz_jobcreator]^7 (server) %s'):format(msg))
    end
end
...
local function canEdit(src)
    ...
    return IsPlayerAceAllowed(src, 'rz.jobcreator')
end
...
RegisterNetEvent('rz_jobcreator:server:list', function()
    local src = source
    TriggerClientEvent('rz_jobcreator:client:jobsList', src, jobs)
end)

RegisterNetEvent('rz_jobcreator:server:save', function(name, jobData)
    local src = source
    if not canEdit(src) then
        TriggerClientEvent('rz_jobcreator:client:error', src, 'Permission refusee.')
        return
    end
    if not validJobName(name) then
        TriggerClientEvent('rz_jobcreator:client:error', src, 'Nom de job invalide (a-z, 0-9, _).')
        return
    end
    local clean = sanitizeJob(jobData)
    if not clean then
        TriggerClientEvent('rz_jobcreator:client:error', src, 'Donnees de job invalides.')
        return
    end
    jobs[name] = clean
    saveToDisk()
    dbg(('job sauve : "%s" (%d grades)'):format(name, count(clean.grades)))
    TriggerClientEvent('rz_jobcreator:client:saved', src, name)
    TriggerClientEvent('rz_jobcreator:client:jobsList', src, jobs)
end)

RegisterNetEvent('rz_jobcreator:server:delete', function(name)
    local src = source
    if not canEdit(src) then
        TriggerClientEvent('rz_jobcreator:client:error', src, 'Permission refusee.')
        return
    ...
    TriggerClientEvent('rz_jobcreator:client:deleted', src, name)
    TriggerClientEvent('rz_jobcreator:client:jobsList', src, jobs)
end)

print('^2[rz_jobcreator]^7 server/main.lua charge — jobs JSON (sans DB) prets')
