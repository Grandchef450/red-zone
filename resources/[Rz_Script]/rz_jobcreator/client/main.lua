-- ============================================================
--  RZ JOB CREATOR  |  client/main.lua
...
local function dbg(msg)
    if Config.Debug then
        print(('^6[rz_jobcreator]^7 %s'):format(msg))
    end
end
...
local function openUI()
    ...
    TriggerServerEvent('rz_jobcreator:server:list')
end
...
print('^2[rz_jobcreator]^7 client charge — /jobcreator ou '..Config.OpenKey)
...
RegisterNUICallback('save', function(data, cb)
    ...
    TriggerServerEvent('rz_jobcreator:server:save', data.name, data.job)
    cb({ ok = true })
end)

RegisterNUICallback('delete', function(data, cb)
    if type(data.name) == 'string' then
        TriggerServerEvent('rz_jobcreator:server:delete', data.name)
    end
    cb({ ok = true })
end)
...
RegisterNetEvent('rz_jobcreator:client:jobsList', function(jobs)
    sendUI('jobsList', { jobs = jobs or {} })
end)

RegisterNetEvent('rz_jobcreator:client:saved', function(name)
    sendUI('saved', { name = name })
end)

RegisterNetEvent('rz_jobcreator:client:deleted', function(name)
    sendUI('deleted', { name = name })
end)

RegisterNetEvent('rz_jobcreator:client:error', function(msg)
...
--
--  Le panneau admin appelle https://rz_jobcreator/openPanel. On relaie sur
