...
local function dbg(msg)
    if Config.Debug then print(('^5[rz_vip]^7 %s'):format(msg)) end
end
...
RegisterCommand('vip', function()
    ...
    TriggerServerEvent('rz_vip:server:requestOpen')
end, false)
RegisterKeyMapping('vip', 'Tablette VIP (RZ)', 'keyboard', Config.OpenKey)

RegisterNetEvent('rz_vip:client:open', function(state)
    isOpen = true
    SetNuiFocus(true, true)
    sendUI('open', { state = state })
end)

RegisterNetEvent('rz_vip:client:state', function(state)
    sendUI('state', { state = state })
end)

RegisterNetEvent('rz_vip:client:result', function(res)
...
RegisterNUICallback('refresh', function(_, cb) TriggerServerEvent('rz_vip:server:refresh') cb('ok') end)
RegisterNUICallback('buyTier', function(d, cb) TriggerServerEvent('rz_vip:server:buyTier', tonumber(d.id)) cb('ok') end)
RegisterNUICallback('choosePed', function(d, cb) TriggerServerEvent('rz_vip:server:choosePed', tostring(d.model)) cb('ok') end)
RegisterNUICallback('applyPed', function(d, cb) TriggerServerEvent('rz_vip:server:applyPed', tostring(d.model)) cb('ok') end)
RegisterNUICallback('useFreeRevive', function(_, cb) TriggerServerEvent('rz_vip:server:useFreeRevive') cb('ok') end)
RegisterNUICallback('craftKit', function(_, cb) TriggerServerEvent('rz_vip:server:craftKit') cb('ok') end)
RegisterNUICallback('useKit', function(_, cb) TriggerServerEvent('rz_vip:server:useKit') cb('ok') end)
RegisterNUICallback('buyShopItem', function(d, cb) TriggerServerEvent('rz_vip:server:buyShopItem', tostring(d.id)) cb('ok') end)
...
RegisterNetEvent('rz_vip:client:applyPed', function(model)
...
RegisterNetEvent('rz_vip:client:reviveHospital', function(hosp)
    DoScreenFadeOut(400)
    Wait(500)
...
local function harvest(index, spot)
    ...
    ClearPedTasks(ped)
    TriggerServerEvent('rz_vip:server:farm', index)
    farming = false
end

RegisterNetEvent('rz_vip:client:farmed', function(material, amount)
...
print('^2[rz_vip]^7 client charge — F4 / commande "vip"')

-- ═══════════════════════════════════════════════════════════════════
--  OUVERTURE DEPUIS LE MENU ADMIN (F5)
--
--  Le panneau admin appelle https://rz_vip/openPanel. On relaie sur
