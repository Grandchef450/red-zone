...
-- Dynasty 8 : agent immobilier = joueur possédant l'ace Config.RealtorAce (admins), pas un job.
function IsRealtorSrc(src)
    return IsPlayerAceAllowed(tostring(src), Config.RealtorAce) == true
end

lib.callback.register('ps-housing:cb:isRealtor', function(src)
    return IsRealtorSrc(src)
end)
