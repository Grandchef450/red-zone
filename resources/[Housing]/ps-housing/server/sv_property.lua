...
function Property:UpdateOwner(data)
    ...
    -- Vente faite par un admin (Dynasty 8) : commission uniquement s'il a réellement un job d'agent.
    local commissionRate = RealtorJobs[realtor.PlayerData.job.name] and Config.Commissions[realtorGradeLevel] or 0
    local commission = math.floor(self.propertyData.price * commissionRate)
...
lib.callback.register('ps-housing:cb:getPropertyInfo', function (source, property_id)
    ...
    if not IsRealtorSrc(src) then return end
