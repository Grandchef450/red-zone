...
local function setRealtor()
	local isAdmin = lib.callback.await('bl-realtor:server:isRealtor', false) == true
	SendNUIMessage({
		action = "setRealtorGrade",
		data = isAdmin and 4 or -1
	})
end
...
local function tryToggleUI()
	if not UIOpen and lib.callback.await('bl-realtor:server:isRealtor', false) ~= true then
		lib.notify({ title = 'Dynasty 8 Real Estate', description = 'Accès réservé aux administrateurs.', type = 'error' })
		return
	end
...
if Config.UseCommand then
	RegisterCommand("housing", tryToggleUI, false)
end

RegisterNetEvent('bl-realtor:client:toggleUI', tryToggleUI)
