...
function IsRealtor(src)
    return IsPlayerAceAllowed(src, Config.RealtorAce)
end

lib.callback.register('bl-realtor:server:isRealtor', function(src)
    return IsRealtor(src)
end)

RegisterNetEvent("bl-realtor:server:updateProperty", function(type, property_id, data)
    -- Job check
    local src = source
    if not IsRealtor(src) then return false end
